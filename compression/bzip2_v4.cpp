...
// Function to compress the input stream in parallel using OpenMP
std::array<size_t, 3> compress_stream(FILE* istream, FILE* ostream) {
    ...
    while (not(feof(istream))) {
        // Store compressed chunks for each thread
        ...
        #pragma omp parallel
        {
            ...
            // Store the compressed chunk for each thread
            compressed_chunks[thread_id] = std::move(odata);
